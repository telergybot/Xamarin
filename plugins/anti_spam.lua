  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "کاربر ["..msg.from.id.."] بدلیل اسپم بلاک شد")
			savelog(msg.from.id.." PM", "کاربر ["..msg.from.id.."] بدلیل اسپم بلاک شد")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "کاربر @"..username.." با کد\n["..msg.from.id.."] بدلیل اسپم اخراج شد")
		else
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked for #spam")
			send_large_msg(receiver , "کاربر "..name_log.." با کد\n["..msg.from.id.."] بدلیل اسپم اخراج شد")
		end
	  end
      -- incr it on redis
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر [ "..name.." ] با کد\n"..msg.from.id.." بدلیل اسپم بن گلوبال شد")
		  send_large_msg("channel#id"..msg.to.id, "کاربر [ "..name.." ]با کد\n"..msg.from.id.." بدلیل اسپم بن گلوبال شد")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." از ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] به دلیل اسپم بن گلوبالی شد"
