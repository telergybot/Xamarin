...
--Edit by : @AmirDark From @DarkTeam -- @XamarinCH
...
if result.username then
	channel_username = "🔢 نام کاربری گروه : @"..result.username
else
	channel_username = ""
end
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'گذاشتن لینک هم اکنون ممنوع است'
  else
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'گذاشتن لینک هم اکنون مجاز است'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'گذاشتن لینک مجاز شد'
  end
end
      ...
      return 'لطفا لینک گروه را ارسال نمایید'
		...
		if matches[1] == 'setflood' then
			...
			if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 20 then
				return "عدد انتخاب شده باید بین 5 تا 20 باشد"
			end
