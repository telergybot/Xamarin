...
local function admin_demote(msg, admin_id)
	...
	save_data(_config.moderation.data, data)
	return admin_id..' از مدیریت عزل شد'
end
...
local function admin_list(msg)
	...
	local message = '👥 لیست مدیران کل ربات :\n'
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
            return send_large_msg(receiver, '@'..member_username..' در حال حاضر یک مدیر است')
        end
        data['admins'][tostring(member_id)] = member_username
        save_data(_config.moderation.data, data)
	return send_large_msg(receiver, '@'..member_username..' مدیر ربات شد')
end
...
local function admin_user_demote(receiver, member_username, member_id)
	...
	if not data['admins'][tostring(member_id)] then
		send_large_msg(receiver, "@"..member_username..' یک مدیر نیست')
		return
    end
	data['admins'][tostring(member_id)] = nil
	save_data(_config.moderation.data, data)
	send_large_msg(receiver, 'مدیر @'..member_username..' از مدیریت عزل شد')
end
...
local function res_user_support(cb_extra, success, result)
   ...
   local support_id = result.peer_id
	if get_cmd == 'addsupport' then
		...
		send_large_msg(receiver, "کاربر ["..support_id.."] به گروه پشتیبانی اضافه شد")
	elseif get_cmd == 'removesupport' then
		support_remove(support_id)
		send_large_msg(receiver, "کاربر ["..support_id.."] از گروه پشتیبانی حذف شد")
	end
end
    ...
    local receiver = get_receiver(msg)
		...
		if matches[1] == 'support' and matches[2] then
			if string.match(matches[2], '^%d+$') then
				...
				support_add(support_id)
				return "کاربر ["..support_id.."] به پشتیبانی اضافه شد"
			else
				local member = string.gsub(matches[2], "@", "")
				local receiver = get_receiver(msg)
		...
		if matches[1] == '-support' then
			if string.match(matches[2], '^%d+$') then
				...
				support_remove(support_id)
				return "کاربر ["..support_id.."] از پشتیبانی حذف شد"
			else
				local member = string.gsub(matches[2], "@", "")
				local receiver = get_receiver(msg)
		...
		if matches[1] == 'list' and matches[2] == 'groups' then
			if msg.to.type == 'chat' or msg.to.type == 'channel' then
				...
				return "لیست گروه ها ایجاد شد" --group_list(msg)
