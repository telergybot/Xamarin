    ...
    if data[tostring(msg.to.id)]['settings']['lock_arabic'] == 'yes' then
	  ...
	  if msg.to.type == 'chat' then
		...
		if username and is_super_group(msg) then
			send_large_msg(receiver , "کاربر @"..username.."["..msg.from.id.."] به دلیل استفاده از عربی یا فارسی از گروه اخراج و پیام هایش پاک شد")
		else
			send_large_msg(receiver , "کاربر "..name.."["..msg.from.id.."] به دلیل استفاده از عربی یا فارسی از گروه اخراج و پیام هایش پاک شد")
		end
