...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "نام کاربری یافت نشد!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "شما نمیتوانید افراد را اخراج کنید")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "شما نمیتوانید مدیران را اخراج کنید")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "شما نمیتوانید مدیران را بن کنید")
			return
        end
        send_large_msg(receiver, '⛔️ کاربر @'..member..'\nبا کد ['..member_id..'] بن شد!')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, '⛔️ کاربر @'..member..'\nبا کد ['..member_id..'] آنبن شد!')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'کاربر '..user_id..' آنبن شد.'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '⛔️ کاربر @'..member..' با کد\n['..member_id..'] بن گلوبال شد!')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '⛔️ کاربر @'..member..' با کد\n['..member_id..'] آنبن گلوبال شد!')
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید مدیران را بن نمایید"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "شما نمیتوانید افراد را بن نمایید"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
		...
		send_large_msg(receiver, '⛔️ کاربر ['..matches[2]..'] بن شد')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == 'unban' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return '⛔️ کاربر '..user_id..' آنبن شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
...
if matches[1]:lower() == 'kick' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید مدیران را اخراج نمایید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '⛔️ کاربر ['..user_id..' ] بن گلوبال شد'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '⛔️ کاربر ['..user_id..' ] آنبن گلوبال شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
